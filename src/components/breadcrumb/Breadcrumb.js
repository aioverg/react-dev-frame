...
import { getrouterMap } from '@router/menu.route'
...
class Breadcrumbs extends PureComponent {
  constructor(props) {
    ...
    this.routerPathMap = getrouterMap('path', props.menuList)
  ...
  componentDidUpdate () {
    ...
    if (menuList && menuList.length != this.state.menuLen) {
      this.routerPathMap = getrouterMap('path', menuList)
  ...
  getItems = (pathname) => {
    ...
    if (paths && paths.length > 2) {
      for (let i = 2; i <= paths.length; i++) {
        ...
        if (this.routerPathMap[p]) {
          const { name, transKey } = this.routerPathMap[p]
          keys.push(p)
          trans.push(transKey)
          names.push(name)
